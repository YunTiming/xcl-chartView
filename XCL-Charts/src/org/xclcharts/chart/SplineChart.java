...
//		System.out.println("setCategories");
	 ...
	 *            定制线数据集合
	 */
	public void setCategoryAxisCustomLines(
			List<CustomLineData> customLineDataset) {
	...
	private void calcAllPoints(SplineData bd, List<PointF> lstPoints,
	...
	private boolean renderBezierCurveLine(Canvas canvas, Path bezierPath,
		...
		renderBezierCurveLine(canvas, spData.getLinePaint(), bezierPath,
				lstPoints);
		return true;
	}

	private boolean renderDotAndLabel(Canvas canvas, SplineData spData,
			int dataID, List<PointF> lstPoints) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (!pLine.getDotStyle().equals(XEnum.DotStyle.HIDE)) {
				PlotDotRender.getInstance().renderDot(canvas, pDot, dotInfo.mX,
						dotInfo.mY, pLine.getDotPaint()); // 标识图形
			...
			if (spData.getLabelVisible()) {
				// 请自行在回调函数中处理显示格式
				spData.getLabelOptions().drawLabel(canvas,
						pLine.getDotLabelPaint(),
						getFormatterDotLabel(dotInfo.getLabel()), dotInfo.mX,
						dotInfo.mY, itemAngle, spData.getLineColor());
			}
		}
	...
	protected void drawClipPlot(Canvas canvas) {
		if (renderPlot(canvas) == true) {
			...
			{
				mXAxisCustomLine.renderCategoryAxisCustomlines(canvas,
						this.getPlotScreenWidth(), this.plotArea, mMaxValue,
						mMinValue);
			}
